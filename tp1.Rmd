...
library(gridExtra) # presentacion de graficos contiguos
...
b <- R/(Lmol*log(P0))+1/T0
m <- -R/(Lmol)
```

$$
\begin{split}
b &= \frac{R\cdot \ln P_0}{L} + \frac{1}{T_0} &\approx 2,6976 \times10^{-3} \text{ K}^{-1} \\
m &= \frac{-R}{L}  &\approx -2,0475 \times10^{-4} \text{ K}^{-1}
\end{split}
$$
...
resumen <- crossing(modelos, datos) %>%
  mutate(
    ...
    R2_ajustado = map_dbl(resumen, "adj.r.squared"),
    p_valor = map_dbl(resumen, "p.value"))

resumen %>%
  select(nombre_modelo, con12, R2_ajustado, p_valor)
```
...
predicciones <- resumen %>%
  ...
  select(nombre_modelo, con12, PA, PE, PEhat) %>%
  unnest() %>%
  mutate(
...
predicciones %>%
  ggplot(aes(PA, resid)) +
    geom_point() +
    facet_grid(con12 ~ nombre_modelo, scales = "free")
```
...
```{r}
blin <- filter(resumen, con12=="no", nombre_modelo=="lineal")$b
mlin <- filter(resumen, con12=="no", nombre_modelo=="lineal")$m
bfis <- filter(resumen, con12=="no", nombre_modelo=="fisico")$b
mfis <- filter(resumen, con12=="no", nombre_modelo=="fisico")$m
tibble(
...
$$
\begin{split}
b &\approx 2,6976 \times10^{-3} \text{ K}^{-1},\quad m&\approx 2,0475 \times10^{-4} \text{ K}^{-1} \\
\hat{b} &\approx `r bfis` \times10^{-3} \text{ K}^{-1},\quad \hat{m}&\approx `r mfis` \times10^{-4} \text{ K}^{-1}
\end{split}
$$
