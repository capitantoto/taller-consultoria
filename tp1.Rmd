...
Asumiendo que el huevo está compuesto por dos medios elipsoides, y la sección transversal es perfectamente circular, su volumen está dado por $V=\pi\times\left(\tfrac{M}{2}\right)^2\times L$:
```{r}
df4 <- read_csv("data/1-4.csv")
df4 <- df4 %>%
  mutate(V_ = pi * (M/2)^2 * L)

df4 %>%
  ggplot(aes(V, V_)) + geom_point()
```
