...
En una primera aproximación _inercial_ (en tanto está influida por la metodología del ejercicio anterior), nos lleva a
...
Pensándolo bien, creeríamos que el problema es autoimpuesto, al usar un test no-paramétrico, para comparar cantidades que están muy cerca entre sí (# de larvas para las emulsiones 2 y 3) contra otras que están muy lejos de ambas (los controles). Diferencias ínfimas entre emulsiones y considerables respecto al control, al pasarlas a la escala de rangos, quedan "emparejadas" en distancias unitarias, y si por azar una emulsión mata alguna larva menos que otra, el p-valor en sus tests de comparaciones múltiples sufrirá más de lo justificado. Para comprobar esta hipótesis, realizamos el mismo test de comparaciones múltiples de antes, pero ordenando los tratamientos siempre igual: Control 1 > Control 2 > Emulsión 1 > Emulsión 2 > Emulsión 3 > Emulsión 4. Los p-valores resultan:
...
df3 <- df3 %>%
  mutate(
    blq = fct_reorder(as_factor(blq), larvas_, mean),
...
Salvo por una observación atípica para la emulsión 4, los datos correspondientes a las 4 emulsiones tiene una distribución aceptablemente similiar a una normal. Para los controles combinados, una distribución normal no parece demasiado adecuada en los extremos, pero tampoco es aberrante, así que por el momento consideramos que los supuestos de normalidad y homocedasticidad se sostienen razonablemente. Ajustamos un modelo de efectos aditivos del estilo $\sqrt{larvas} \sim bloque + emulsión$, y examinamos los resultados:
...
Se observa inmediatamente que el modelo produce un buen ajuste, y que el efecto de los tratamiento esindudablemente significativo, tanto conjuntamente como individualmente. El efecto de los bloques es significativo al tradicional nivel del 5%, por lo que hemos decidido conservarlo en el modelo, pero obviar los bloques podría ser una decisión razonable también. Graficando los residuos estudentizados contra las cantidades predichas, no nos encontramos con ninguna estructura remanente, pero sí observamos un par de _outliers_ con residuo mayor a 3 correspondientes al control. Esto era de esperar, ya que como vimos en los QQ-plots por población, el control se alejaba de la normalidad en los extremos. 
...
Consideramos rehacer el análisis sin estas observaciones potencialmente atípicas, pero las mejoras en el $R^2$ ajustado del modelo, y la significatividad de los coeficientes eran más bien menores, por lo cual preferimos mantener el _dataset_ completo. Finalmente, estamos en condiciones de realizar un test de Tukey de comparaciones múltiples _entre tratamientos_, que arroja los siguientes resultados:
```{r}
...
- la emulsión 1 es significativamete más efectiva que las 3 y 4, pero que la dos.
