...
Para testear la normalidad de los datos, comensamos por graficar la estimación empírica de la densidad de cada tipo de maíz, y de la diferencia de alturas por maceta:
...
g1 <- df1 %>%
  ...
  geom_density() +
  labs(title = "Densidad empírica: diferencia")

g2 <- df1 %>%
  gather("variedad", "altura", -maceta) %>%
  ggplot(aes(altura, color = variedad)) +
  geom_density() +
  labs(title = "Densidades empíricas: especies") +
...
Hasta ahora, hemos concluido que bajo una transformación $g$ de los datos,
- las observaciones son aproximadamente homocedásticas, y
- un modelo multiplicativo no aporta más que el aditivo para el análisis.
...
La inferencia mas allá de estas conclusiones es algo limitada. En primer lugar, al no contar con un control, ni siquiera podemos argumentar que alguno de los tratamientos más efectivo que un simple placebo. Por otra parte, tampoco tenemos una forma directa de responder _por cuánto_ mejora la sobrevida en términos absolutos en la escala original. Lo que sí podemos dar, es tomar $g^{-1}(\hat{\mu}+\hat{\alpha}_i+\hat{\beta}_j)$ como una aproximación de la sobrevida media para cada combinación de venenos y tratamientos. Ordenadas por $g^{-1}$, tenemos:
...
$$
p_i = \pi + \beta_1 \: d_{A,i} + \beta_2 \:(d_{A,i})^2 + \beta_3 \:d_{B,i} + \beta_4 \:d_B + \beta_5 \: n_i +\epsilon_i \quad \text{con } \epsilon_i \sim N(0, \sigma^2)
$$
...
augment(lm3, newdata = grilla) %>%
  ...
  labs(title = "Predicción de la roporción de células aberrantes para tamaños de muestra seleccionados",
       linetype = "n")
```