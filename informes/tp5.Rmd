...
Mas allá de un potencial outlier en el gráfico de residuos versus predichos, el modelo aprece ajustar más que adecuadamente. Antes de concluir, una palabra de cautela: en casos como este, en que hay más predictores que variables, las técnicas de validación cruzada no garantizan elegir el "mejor" modelo, ya que lo que conseguimos es más bien el modelo mejor ajustado al conjunto de entrenamiento. Para ilustrarlo, a continuación presentamos los "mejores modelos" obtenidos con este mismo proceso, en 10 procedimientos idénticos salvo por la elección aleatoria de los conjuntos de prueba y entrenamiento. El lector inquieto puede comprobar personalmente que los gráficos de diagnóstico son sumamente buenos para todos ellos:

```{r}
res %>%
  filter(ciclo %in% c(1, 5:10)) %>% 
  group_by(ciclo) %>%
  top_n(1, -mse_test) %>%
  ungroup() %>%
  select(grupo, nv, mse_test, mse_train, f) %>%
