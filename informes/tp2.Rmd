...
2. **Análisis de residuos**: Elegido el mejor modelo según la R1DE, revisaremos los residuos de predicción para comprobar que (a) no posean ninguna estructura obvia y (b) sean aproximadamente normales. De violarse severamente alguno de estos supuestos, deberemos descartar la elección y repetir este paso para el siguiente mejor modelo. Si volvemos a ver comportamientos sospechosos, procederemos a hacer un análisis de outliers, descartar las observaciones culpables y repetir la etapa de selección de modelos.
...
computar_distancias <- function(data, vars, tol = 1) {
  p <- length(vars)
  n <- nrow(data)
  covrob <- covRob(data[,vars])
  .data %>%
    mutate(robdist = covrob$dist) %>%
    arrange(robdist) %>%
    mutate(
      espdist = qchisq(stats::ppoints(n), df = p),
      outlier = abs(robdist - espdist)/espdist > tol)
}

qq_dists <- function(data, vars, tol = 1) {
  p <- length(vars)
  if (!"id" %in% names(data)) {
    data <- rowid_to_column(data, "id")
  }
  data <- computar_distancias(data, vars, tol = tol)
  data %>%
    ggplot(aes(espdist, robdist, label = id)) +
    geom_qq_line(aes(sample = robdist), linetype = "dotted",
                 distribution = qchisq, dparams = list(df = p)) +
    geom_point() +
    geom_label(data = filter(data, outlier)) +
    labs(title = paste0("QQ-plot para las distancias de Mahalanobis robustas, p = ", p),
         y = "Distancia Robusta Muestral", x = "Cuantil esperado")
}
```
...
A continuación, presentamos en forma de tabla el ECM de LOOCV (y su desvio) para los mejores de entre una selección arbitratria de modelos posibles, que incluye todos los modelos aditivos y multiplicativos de 2, 3 y 4 covariables:
...
- un _scatterplot_ de los residuos estudentizados contra los valores predichos y
...
No se observa estructura evidente ni valores extremos de los residuos estudentizados al graficarlos contra predichos. Tampoco hay grandes alejamientos de la normalidad en el QQ-plot, así que confirmamos la selección previa (consideramos remover la observación #10, pero la elección de modelo y los resultados finales eran idénticos). Finalmente, reportamos los principales estadísticos del modelo $R \sim A + V$ ajustado _con todas las observaciones_ junto con los valores de sus coeficientes:
...
Consideramos los modelos aditivos y multiplicativos para 2 y 3 variables, además de todos los modelos de 1 sola covariable basados en (a) una variable original, (b) la interacción de algunas de ellas o (c) el cuadrado o cubo de alguna de ellas. Los ECM estimados por LOOCV para los 10 mejores modelos resultan ser:
```{r_modelos}
modelos2 <- list(
  cte = Y ~ 1,
  X1X2 = Y ~ X1 + X2,
...
Casualmente, el modelo de menor ECM es también uno de mínima complejidad (el modelo de sólo ordenada es el de peor ECMentre los evaluados), así que en principio, $\omega_{abs} \equiv \omega_{1sd} := Y \sim X_1\times X_2$.
...
Esta vez, sí observamos cierta estructura en los residuos (que parecieran formas dos "clusters"), pero ninguna tendencia obvia. Si realizamos el mismo gráfico para otros modelos al azar, se nota esta misma seudo-agrupación, así que por el momento no hacemos nada más.
...
Repetimos este proceso iterativamente, eliminando primero la observación #9, y luego la #7. En ambos casos, $Y \sim X_1 + X_2$, resulta ser el modelo con menor ECM de LOOCV absoluto, e $Y \sim X_1\times X_2$ sigue siendo el mejor modelo según la R1DE. Sin las observaciones $(21, 9, 7)$, el scatterplot de residuos y la recta cuantil-cuantil no muestran mayores irregularidades:
...
Cuando comparamos tanto los estadísticos globales como los coeficientes particulares del modelo ajustado con y sin las observaciones $(21, 9, 7)$, observamos una sutil mejora en el $R^2_{aj}$, pero coeficientes iguales hasta el cuarto decimal en ambos casos. Haciendo la misma salvedad sobre la interpretabilidad de los p-valores, a continuación reportamos los resultados del modelo ajustado _con todas las observaciones_ para el modelo elegido, $Y \sim X_1 \times X_2$:
...
Esta vez, sí observamos cierta estructura en los residuos (que parecieran formas dos "clusters") y un potencial candidato a outlier en la obs. #18, pero ninguna tendencia obvia. Si realizamos el mismo gráfico para otros modelos al azar, se nota esta misma seudo-agrupación, así que por el momento no hacemos nada más.

```{r}
plot(lm3, which = 2, id.n = 5)
outs <- c(18, 19, 8, 7, 9)
res3 <- resumir(df3[-outs,], "Y", modelos3)
imprimir_resumen(res3)
lm3.2 <- lm(Y ~ L3 + L4, df3[-outs,])
plot(lm3.2, which = 2, id.n = 3, labels.id = df3[-outs,]$id)
map_df(lst(lm3, lm3.2), tidy)
```

En el QQ-plot de los residuos, seulta evidente que algunas observaciones se alejan considerablemente de los cuantiles esperados. Seguimos el mismo procedimiento de antes de ir eliminando observaciones y recompuntado el ECM para encontrar el mejor modelo según la R1DE, que resulta en la siguiente secuencia de eliminaciń de observaciones: $(18, 19, 8, 7, 9)$. En cada caso, el mejor modelo, tanto en ECM absoluto como según la R1DE siguió siendo $Y \sim L_3 + L_4$, y los coeficientes finales no variaron demasiado:
...
Al igual que antes, concluimos con los estadísticos principales y coeficientes para el modelo elegido ajustado con todas las observaciones, $Y \sim L_3 + L_4$
