...
<!-- Lamentablemente, la matriz de covarianza robusta entre todas las variables de interés (G, P, A, V y R) resulta singular, probablemente a causa de los valores repetidos de las tuplas (G, P, A). En su defecto, calcularemos la covrianza robusta para las 4 variables G, P, V, R: los resultados son muy similares si se usan (G, A, V, R) o (P, A, V, R). No se observan grandes desvíos de la distribución $\chi^2_4$ esperada, así que proseguimos el análisis con todos los datos. -->
...
<!-- qq_dists(df1, c("G", "P", "V", "R"), tol = 1.5) -->
<!-- ``` -->
...
ggplot(df1, aes(sample = .resid)) +
  geom_qq() +
  geom_qq_line() +
  labs(title = "QQ-plot de los residuos de predicción",
       y = "Residuo", x = "Cuantil teórico")
```

No se observa estructura evidente ni valores extremos de los residuos estudentizados al graficarlos contra predichos. Tampoco hay grandes alejamientos de la normalidad en el QQ-plot, así que confirmamos la selección previa. Finalmente, reportamos los principales estadísticos del modelo $R \sim A + V$ junto con los valores de sus coeficientes:
...
# ```{r ej2}
# vars2 <- c("X1", "X2", "X3", "Y")
# qq_dists(df2, vars2, tol = 2)
# ```
# 
# Aquí se nota claramente que las observaciones 1, 2, 3, 4 y 21 tienen distancias robustas mucho más extremas de las que uno esperaría para una muestra de este tamaño. Para darnos una idea de a qué puede deberse una discrepancia tan considerable, realizamos los gráficos de dispersión para cada par de variables junto con la estimación de la densidad empírica para cada una, coloreando en celeste los potenciales outliers detectados.
# 
# ```{r}
# df2$outlier <- df2$id %in% c(1:4, 21)
# ggpairs(df2, columns = vars2, mapping = aes(color = outlier),
#         upper = list(continuous = "points"), lower = list(continuous = "blank"))
# ```
...
La buena noticia, es que salvo un ligero desvío para las observaciones con máximo y mínimo residuo, el QQ-plot luce razonable, así que damos por aceptable el análisis de residuos.

Al igual que antes, concluimos con los estadísticos principales y coeficientes para el modelo elegido, $Y \sim X_1 \times X_2$
...
```{r ej2}
qq3 <- ggplot(df3, aes(sample = .resid)) +
  geom_qq() +
  geom_qq_line() +
  labs(title = "QQ-plot de los residuos de predicción",
       y = "Residuo", x = "Cuantil teórico")
